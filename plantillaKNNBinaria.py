...
# Se introducen 5 argumentos: k , d , w , salvar(-s) , aplicar procesado (-p)
...
# Finalmente, se define una función para convertir las variables a Unicode, dependiendo de la versión de Python que se esté utilizando. """
    ...
    if(sys.argv[4]=='-e' or sys.argv[4]=='-s'):
    ...
    # que convierte la fecha y hora en segundos transcurridos desde el 1 de enero de 1970.
        ...
        print("la k es:" +sys.argv[1])
        print("la d es:" + str(sys.argv[2]))
        clf = KNeighborsClassifier(n_neighbors=int(sys.argv[1]),
                            weights=sys.argv[3],
                            algorithm='auto',
                            leaf_size=30,
                            p=int(sys.argv[2]))
...
# Build scored dataset
    ...
    fscore= f1_score(testY, predictions, average='macro')
    print('el fscore : '+ str(fscore))
    ...
    if(sys.argv[4]=='-s'):
        ...
        if os.path.isfile('resultados.csv'):
            ...
            data = {'k': [sys.argv[1]], 'd': [sys.argv[2]], 'Weights': [sys.argv[3]],'precision': [precision],'recall': [recall],'f-score':[fscore]}
